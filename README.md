...
Flake8 was used (run locally on my IDE) to check the Python code. The procedure for this
is to pip install flake8 and then run the command flake8 --exclude=venv,migrations
(which excludes automatically-generated venv and migrations files).
...
- How could someone break it/what happens if you try for functionality involving user inputs or URLs
...
[See detailed steps here](deployment-1.txt)

**Deployment once the project has been created**

- Create a Heroku app

- Create an external database using Elephant SQL 
...
(Note: I didn't realise that my version of Django wasn't compatible with the free tier of ElephantSQL, so I have
